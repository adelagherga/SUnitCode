...
function FPRestrictionsVKMTest(S,U,p)
    ...
    LatticePtsToBeSorted:= [];   // stores the determinant of each matrix; this matrix is associated to a corresponding lattice
    ...
    */
    ...
    changes:= true;
    while (fp ge 0) and (changes eq true) do
        originalF:= F;
        n:= s-1;    // number of columns of B;
        
        m:= Ceiling( Log( (( Pi(RealField())^(n/2) )/( Gamma(n/2+1) ))*(bound^(n/2)) )/Log(p) );   // determines the optimal volume such that V/(covol) < 1 (ie. choose the optimal m)
        ...
        //m:= Log( (( Pi(RealField())^(n/2) )/( Gamma(n/2+1) ))*(bound^(n/2)) )/Log(Determinant(B));   // determines the optimal volume such that V/(covol) < 1 (ie. choose the optimal m)
